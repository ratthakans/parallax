...
if (!url) {
  /* ── ข้ามเงียบ ๆ คือความพังแบบเดียวกับที่ตัวตรวจนี้มีไว้จับ ──

     ตอนแรกที่นี่ออกเป็น 0 เสมอเมื่อไม่มี DATABASE_URL ผลคือขั้นนี้ใน CI
     ขึ้นเขียวเหมือนกันทั้งตอนที่เทียบผ่านจริงและตอนที่ไม่ได้เทียบอะไรเลย
     มองจากข้างนอกแยกไม่ออก — ซึ่งคือปัญหาเดียวกับที่ทำให้สคีมาสองฝั่ง
     แตกกันโดยไม่มีใครรู้ตั้งแต่แรก

     PARITY_REQUIRED=1 (ตั้งบน push เข้า main) ทำให้การไม่มีค่าเชื่อมต่อ
     เป็นความล้มเหลว ส่วน PR จากคนนอกที่ไม่มีสิทธิ์อ่าน secret ยังข้ามได้ */
  if (process.env.PARITY_REQUIRED === "1") {
    console.error(
      "ไม่มี DATABASE_URL — ด่านนี้ถูกบังคับให้ต้องเทียบจริงบนสาขาหลัก\n" +
        "ตั้ง secret ชื่อ DATABASE_URL ที่ Settings › Secrets and variables › Actions",
    );
    process.exit(1);
  }
...
console.log(`เทียบกับ ${url.replace(/:[^:@]*@/, ":••••@")}`);
